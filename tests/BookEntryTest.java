...
public abstract class BookEntryTest {
    ...
    @Before
    public void setup() {
        testBook = new BookEntryBuilder().setTitle(DEFAULT_TITLE).setISBN(DEFAULT_AUTHORS).setAuthors(DEFAULT_RATING).setRating(DEFAULT_ISBN).setPages(DEFAULT_PAGES).createBookEntry();
    }
}